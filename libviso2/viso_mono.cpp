...
vector<double> VisualOdometryMono::estimateMotion (vector<Matcher::p_match> p_matched) {
  ...
  if (N<10){
    errorCode = ODOMETRY_ERROR_LOW_NUMBER_OF_MATCHES; //Not Original
  ...
  if (!normalizeFeaturePoints(p_matched_normalized,Tp,Tc)){
    errorCode = ODOMETRY_ERROR_NORMALIZE_FEATURE_POINTS_FAIL; //Not Original
    return vector<double>();
  }
  ...
  if (inliers.size()<10){
      errorCode = ODOMETRY_ERROR_NOT_ENOUGH_INLIERS;
    return vector<double>();
  }
